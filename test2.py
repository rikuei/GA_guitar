...
def crossover(parent, pop):
    if np.random.rand() < CROSS_RATE:
        ...
        j_ = np.random.randint(0, DNA_SIZE, size=1)
        cross_points = np.hstack((np.zeros(j_), np.ones(DNA_SIZE-j_))).astype(np.bool)
    ...
    for point in range(POP_SIZE):
        for i in range(DNA_SIZE):
            if np.random.rand() < MUTATION_RATE:
                pop[point][i] = np.random.randint(1, 5)
                # if child[point] == 1:
                #     child[point] = 2
                # elif child[point] == 2:
                #     child[point] = 3
