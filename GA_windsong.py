...
j_d = np.zeros((len(judge_dupl), sum(judge_dupl)))
...
def fix_fitness(f, pop):
    for dna in range(POP_SIZE):
        count, hand_cost, cnt, hand_pos = 0, 0, 0, 0
        ...
        for dupl in j_d:
            ...
            if  2 in ddupl:
                hand_pos = frets[ddupl.index(2)]
            elif 1 in ddupl:
                hand_pos = frets[ddupl.index(1)] + 1
            elif 3 in ddupl:
                hand_pos = frets[ddupl.index(3)] - 1
            else :
                hand_pos = frets[ddupl.index(4)] - 2
            if cnt > 0:
                hand_cost = abs(temp - hand_pos)
            cnt += 1
            temp = hand_pos
            if len(fidx) == 2:
